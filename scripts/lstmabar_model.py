"""
LSTMABAR - Language-to-Sound Transformation Model
Updated to use Macro-Control Architecture (Predictor -> Latents -> Decoder -> Engine)
"""
...
# --- Import Existing Towers (Assumed to be in the same folder) ---
...
class LSTMABAR(nn.Module):
    """
    Complete LSTMABAR model with Macro-Control Generation
    
    Flow:
    1. Text/Audio Input -> Encoders -> Embeddings (768 dim)
    2. Embeddings -> Macro Predictor -> Latents (32 dim: Timbre, Envelope, Mod)
    3. Latents -> Parameter Decoder -> DSP Params (Harmonics, Noise, ADSR)
    4. DSP Params -> Harmonic Engine -> Audio Waveform
    """
    ...
    def __init__(
        ...
        # ---------------------------------------------------------
        # 1. The Backbone (Semantic Understanding)
        # ---------------------------------------------------------
        ...
        self.contrastive_aligner = ContrastiveAlignmentModule(
            embedding_dim=embedding_dim,
            device=device
        )
        
        # ---------------------------------------------------------
        # 2. The Generator (Sound Design)
        # ---------------------------------------------------------
        
        # A. The Brain: Predicts abstract sound concepts (32 dims)
        print("🧠 Initializing Macro Archetype Predictor...")
        self.predictor = MacroArchetypePredictor(
            embedding_dim=embedding_dim,
            hidden_dims=[512, 256],
            device=device
        )
        
        # B. The Translator: Expands concepts into physics (100+ params)
        print("Translation Layer: Parameter Decoder...")
        self.decoder = ParameterDecoder(
            input_dim=32,       # 16 Timbre + 8 Env + 8 Mod
            n_harmonics=64,     # Granularity of additive synthesis
            n_noise_bands=32,   # Granularity of subtractive synthesis
            device=device
        )
        
        # C. The Voice: Generates the waveform
        print("🎹 Initializing Harmonic DDSP Engine...")
        self.engine = HarmonicDDSPEngine(
            sample_rate=sample_rate,
            n_harmonics=64,
            device=device
        )
    ...
    def forward(self, descriptions: List[str], audio: torch.Tensor, generate_audio=False):
        """
        Forward pass.
        ...
        Args:
            ...
            generate_audio: If False, skips the decoder/engine (used for contrastive training)
        """
        # 1. Encode Semantics
        ...
        # 2. Generate Sound (Optional)
        if generate_audio:
            # A. Predict Macros
            macros = self.predictor(text_emb, audio_emb)
            latents = macros['combined'] # (Batch, 32)
            
            # B. Decode to DSP Params
            harmonic_dist, noise_bands, adsr, gain = self.decoder(latents)
            
            # C. Synthesize
            generated_audio = self.engine(audio, harmonic_dist, noise_bands, adsr, gain)
            
            output['latents'] = macros
            output['dsp_controls'] = {
                'harmonics': harmonic_dist,
                'noise': noise_bands, 
                'adsr': adsr, 
                'gain': gain
            }
            output['audio_output'] = generated_audio
            
        return output

    def inference(self, description, input_audio):
        """Helper for simple inference calls"""
        self.eval()
        with torch.no_grad():
            if isinstance(description, str): description = [description]
            return self.forward(description, input_audio, generate_audio=True)
...
#         # 1. Macro Predictor (The "Brain")
#         self.predictor = MacroArchetypePredictor(
#             embedding_dim=embedding_dim,
#             device=device
#         )
        
#         # 2. Parameter Decoder (The "Translator")
#         self.decoder = ParameterDecoder(
#             input_dim=32, # 16+8+8 latents
#             n_harmonics=64,
#             device=device
#         )
        
#         # 3. Engine (The "Voice Box")
#         self.engine = HarmonicDDSPEngine(
#             sample_rate=sample_rate, 
#             n_harmonics=64, 
#             device=device
